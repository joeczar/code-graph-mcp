...
function getWorkerPath(): string {
  ...
  // Worker is in @code-graph/core package
  // Navigate from mcp-server/src/tools to core/src/tasks
  return path.resolve(currentDir, '../../../core/dist/tasks/parse-worker.js');
}
...
export const parseDirectoryStartTool: ToolDefinition<typeof parseDirectoryStartInputSchema> = {
  ...
  handler: async (input) => {
    ...
    // Create progress log path
    const task = createParseTask(checkpointDb, {
      directory_path: resolvedPath,
      ...(pattern && { pattern }),
    });

    // Generate progress log path
    const progressLogPath = getLogPath(task.id);

    // Update task with progress log path (we need to do this separately since createParseTask doesn't set it from getLogPath)
    ...
    try {
      ...
      // Clean up task on worker startup failure
      checkpointDb.prepare('DELETE FROM parse_tasks WHERE id = ?').run(task.id);
